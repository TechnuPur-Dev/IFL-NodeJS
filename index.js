...
const BASE_URL = 'https://oracleapex.com/ords/recruit360/sync_data/';
...
// Middleware endpoint that forwards to LocalToCloudSync
app.post('/LocalToCloudSync', async (req, res) => {
  try {
    const resumeData = req.body;
    const subUrl = req.query.subUrl;
    ...
    console.log(`Forwarding request to ${BASE_URL}/LocalToCloudSync?subUrl=${subUrl}`);
    console.log('Request body:', JSON.stringify(resumeData, null, 2));

    // Make the POST request to the external API
    const response = await axios.post(
      `${BASE_URL}LocalToCloudSync`,
      resumeData,
      {
        params: { subUrl },
        headers: {
